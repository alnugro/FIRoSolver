...
import time
from Test.pb_to_sat_conversion.sat_variable_handler import VariableMapper
from Test.pb_to_sat_conversion.pb2cnf import PB2SAT
...
class FIRFilter:
    ...
    def runsolver(self):
        ...
        self.freq_upper_lin = [int(sf.db_to_linear(f)) if not np.isnan(sf.db_to_linear(f)) else np.nan for f in self.freq_upper]
        self.freq_lower_lin = [int(sf.db_to_linear(f)) if not np.isnan(sf.db_to_linear(f)) else np.nan for f in self.freq_lower]
