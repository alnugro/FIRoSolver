...
class FIRFilterPysat:
    ...
    def generate_cnf(self, adderm, h_zero_count, cnf_filename):
        self.max_adder = adderm
        self.h_res = []
        self.gain_res = 0
        half_order = self.half_order - 1  # Adjusting the half-order index

        print("Generating CNF clauses for external solver")
        var_mapper = VariableMapper(half_order, self.wordlength, self.adder_wordlength, self.max_adder, self.adder_depth)
        ...
        print(f"Collecting CNF clauses to write to {cnf_filename}")

        cnf_clauses = []

        # Bound the gain to upper and lower bounds
        gain_literals = [v2i(('gain', g)) for g in range(self.wordlength)]

        # Round to the given word length
        self.gain_upperbound = r2b.frac2round([self.gain_upperbound], self.wordlength, self.fracW, True)[0]
        self.gain_lowerbound = r2b.frac2round([self.gain_lowerbound], self.wordlength, self.fracW, True)[0]

        gain_weight = [1]
        cnf1 = pb2cnf.atleast(gain_weight, [gain_literals], self.gain_lowerbound, self.fracW)
        cnf_clauses.extend(cnf1)

        cnf2 = pb2cnf.atmost(gain_weight, [gain_literals], self.gain_upperbound, self.fracW)
        cnf_clauses.extend(cnf2)

        if h_zero_count > 0:
            print("Processing h_zero_count constraints")
            h_zero_sum = []
            for m in range(half_order + 1):
                for word in range(self.wordlength):
                    cnf_clauses.append([-v2i(('h_zero', m)), -v2i(('h', m, word))])
                h_zero_sum.append(v2i(('h_zero', m)))

            cnf_h_zero = pb2cnf.equal_card(h_zero_sum, h_zero_count)
            cnf_clauses.extend(cnf_h_zero)
        ...
        for omega in range(len(self.freqx_axis)):
            ...
            # Generate CNF for upper bound
            filter_upper_pb_weights = filter_weights + gain_freq_upper_prod_weights
            filter_upper_pb_weights = r2b.frac2round(filter_upper_pb_weights, self.weight_wordlength, self.fracW, True)
            filter_upper_pb_literals = filter_literals + gain_upper_literals

            if len(filter_upper_pb_weights) != len(filter_upper_pb_literals):
                raise Exception("Mismatch in upper filter PB weights and literals")

            cnf3 = pb2cnf.atmost(weight=filter_upper_pb_weights, lits=filter_upper_pb_literals, bounds=0, fracW=self.fracW)
            cnf_clauses.extend(cnf3)

            # Generate CNF for lower bound
            filter_lower_pb_weights = filter_weights + gain_freq_lower_prod_weights
            filter_lower_pb_weights = r2b.frac2round(filter_lower_pb_weights, self.weight_wordlength, self.fracW, True)
            filter_lower_pb_literals = filter_literals + gain_lower_literals

            if len(filter_lower_pb_weights) != len(filter_lower_pb_literals):
                raise Exception("Mismatch in lower filter PB weights and literals")

            cnf4 = pb2cnf.atleast(weight=filter_lower_pb_weights, lits=filter_lower_pb_literals, bounds=0, fracW=self.fracW)
            cnf_clauses.extend(cnf4)

        # Bitshift SAT constraints
        for w in range(1, self.adder_wordlength):
            cnf_clauses.append([-v2i(('c', 0, w))])

        cnf_clauses.append([v2i(('c', 0, 0))])

        for i in range(1, self.max_adder + 1):
            cnf_clauses.append([v2i(('c', i, 0))])

        for w in range(self.adder_wordlength):
            cnf_clauses.append([-v2i(('c', self.max_adder + 1, w))])
        ...
        for i in range(1, self.max_adder + 1):
            ...
            for a in range(i):
                for word in range(self.adder_wordlength):
                    cnf_clauses.append([-v2i(('alpha', i, a)), -v2i(('c', a, word)), v2i(('l', i, word))])
                    cnf_clauses.append([-v2i(('alpha', i, a)), v2i(('c', a, word)), -v2i(('l', i, word))])
                    cnf_clauses.append([-v2i(('beta', i, a)), -v2i(('c', a, word)), v2i(('r', i, word))])
                    cnf_clauses.append([-v2i(('beta', i, a)), v2i(('c', a, word)), -v2i(('r', i, word))])
                alpha_lits.append(v2i(('alpha', i, a)))
                beta_lits.append(v2i(('beta', i, a)))

            cnf5 = pb2cnf.equal_card_one(alpha_lits)
            cnf_clauses.extend(cnf5)

            cnf6 = pb2cnf.equal_card_one(beta_lits)
            cnf_clauses.extend(cnf6)
        ...
        for i in range(1, self.max_adder + 1):
            ...
            for k in range(self.adder_wordlength - 1):
                for j in range(self.adder_wordlength - 1 - k):
                    cnf_clauses.append([-v2i(('gamma', i, k)), -v2i(('l', i, j)), v2i(('s', i, j + k))])
                    cnf_clauses.append([-v2i(('gamma', i, k)), v2i(('l', i, j)), -v2i(('s', i, j + k))])
                gamma_lits.append(v2i(('gamma', i, k)))

            cnf7 = pb2cnf.equal_card_one(gamma_lits)
            cnf_clauses.extend(cnf7)

            for kf in range(1, self.adder_wordlength - 1):
                for b in range(kf):
                    cnf_clauses.append([-v2i(('gamma', i, kf)), -v2i(('s', i, b))])
                    cnf_clauses.append([-v2i(('gamma', i, kf)), -v2i(('l', i, self.adder_wordlength - 1)), v2i(('l', i, self.adder_wordlength - 2 - b))])
                    cnf_clauses.append([-v2i(('gamma', i, kf)), v2i(('l', i, self.adder_wordlength - 1)), -v2i(('l', i, self.adder_wordlength - 2 - b))])

            cnf_clauses.append([-v2i(('l', i, self.adder_wordlength - 1)), v2i(('s', i, self.adder_wordlength - 1))])
            cnf_clauses.append([v2i(('l', i, self.adder_wordlength - 1)), -v2i(('s', i, self.adder_wordlength - 1))])
        ...
        for i in range(1, self.max_adder + 1):
            for word in range(self.adder_wordlength):
                cnf_clauses.extend([
                    [-v2i(('delta', i)), -v2i(('s', i, word)), v2i(('x', i, word))],
                    [-v2i(('delta', i)), v2i(('s', i, word)), -v2i(('x', i, word))],
                    [-v2i(('delta', i)), -v2i(('r', i, word)), v2i(('u', i, word))],
                    [-v2i(('delta', i)), v2i(('r', i, word)), -v2i(('u', i, word))],
                    [v2i(('delta', i)), -v2i(('s', i, word)), v2i(('u', i, word))],
                    [v2i(('delta', i)), v2i(('s', i, word)), -v2i(('u', i, word))],
                    [v2i(('delta', i)), -v2i(('r', i, word)), v2i(('x', i, word))],
                    [v2i(('delta', i)), v2i(('r', i, word)), -v2i(('x', i, word))]
                ])

        # Constraints involving epsilon, x, y, z, cout, etc.
        for i in range(1, self.max_adder + 1):
            for word in range(self.adder_wordlength):
                cnf_clauses.extend([
                    [v2i(('u', i, word)), v2i(('epsilon', i)), -v2i(('y', i, word))],
                    [v2i(('u', i, word)), -v2i(('epsilon', i)), v2i(('y', i, word))],
                    [-v2i(('u', i, word)), v2i(('epsilon', i)), v2i(('y', i, word))],
                    [-v2i(('u', i, word)), -v2i(('epsilon', i)), -v2i(('y', i, word))]
                ])

        # Adder logic constraints
        for i in range(1, self.max_adder + 1):
            # Sum and carry logic at bit 0
            cnf_clauses.extend([
                [v2i(('x', i, 0)), v2i(('y', i, 0)), v2i(('epsilon', i)), -v2i(('z', i, 0))],
                [v2i(('x', i, 0)), v2i(('y', i, 0)), -v2i(('epsilon', i)), v2i(('z', i, 0))],
                [v2i(('x', i, 0)), -v2i(('y', i, 0)), v2i(('epsilon', i)), v2i(('z', i, 0))],
                [-v2i(('x', i, 0)), v2i(('y', i, 0)), v2i(('epsilon', i)), v2i(('z', i, 0))],
                [-v2i(('x', i, 0)), -v2i(('y', i, 0)), -v2i(('epsilon', i)), v2i(('z', i, 0))],
            ...
            # Carry logic at bit 0
            cnf_clauses.extend([
                [-v2i(('x', i, 0)), -v2i(('y', i, 0)), v2i(('cout', i, 0))],
                [v2i(('x', i, 0)), v2i(('y', i, 0)), -v2i(('cout', i, 0))],
                [-v2i(('x', i, 0)), -v2i(('epsilon', i)), v2i(('cout', i, 0))],
                [v2i(('x', i, 0)), v2i(('epsilon', i)), -v2i(('cout', i, 0))],
                [-v2i(('y', i, 0)), -v2i(('epsilon', i)), v2i(('cout', i, 0))],
                [v2i(('y', i, 0)), v2i(('epsilon', i)), -v2i(('cout', i, 0))]
            ])

            # Sum and carry logic for bits > 0
            for kf in range(1, self.adder_wordlength):
                # Sum logic
                cnf_clauses.extend([
                    [v2i(('x', i, kf)), v2i(('y', i, kf)), v2i(('cout', i, kf - 1)), -v2i(('z', i, kf))],
                    [v2i(('x', i, kf)), v2i(('y', i, kf)), -v2i(('cout', i, kf - 1)), v2i(('z', i, kf))],
                    [v2i(('x', i, kf)), -v2i(('y', i, kf)), v2i(('cout', i, kf - 1)), v2i(('z', i, kf))],
                    [-v2i(('x', i, kf)), v2i(('y', i, kf)), v2i(('cout', i, kf - 1)), v2i(('z', i, kf))],
                    [-v2i(('x', i, kf)), -v2i(('y', i, kf)), -v2i(('cout', i, kf - 1)), v2i(('z', i, kf))],
                ...
                # Carry logic
                cnf_clauses.extend([
                    [-v2i(('x', i, kf)), -v2i(('y', i, kf)), v2i(('cout', i, kf))],
                    [v2i(('x', i, kf)), v2i(('y', i, kf)), -v2i(('cout', i, kf))],
                    [-v2i(('x', i, kf)), -v2i(('cout', i, kf - 1)), v2i(('cout', i, kf))],
                    [v2i(('x', i, kf)), v2i(('cout', i, kf - 1)), -v2i(('cout', i, kf))],
                    [-v2i(('y', i, kf)), -v2i(('cout', i, kf - 1)), v2i(('cout', i, kf))],
                    [v2i(('y', i, kf)), v2i(('cout', i, kf - 1)), -v2i(('cout', i, kf))]
                ])

            # Additional constraints at the most significant bit
            cnf_clauses.extend([
                [v2i(('epsilon', i)), v2i(('x', i, self.adder_wordlength - 1)), v2i(('u', i, self.adder_wordlength - 1)), -v2i(('z', i, self.adder_wordlength - 1))],
                [v2i(('epsilon', i)), -v2i(('x', i, self.adder_wordlength - 1)), -v2i(('u', i, self.adder_wordlength - 1)), v2i(('z', i, self.adder_wordlength - 1))],
                [-v2i(('epsilon', i)), v2i(('x', i, self.adder_wordlength - 1)), -v2i(('u', i, self.adder_wordlength - 1)), -v2i(('z', i, self.adder_wordlength - 1))],
                [-v2i(('epsilon', i)), -v2i(('x', i, self.adder_wordlength - 1)), v2i(('u', i, self.adder_wordlength - 1)), v2i(('z', i, self.adder_wordlength - 1))]
            ])

        # Further constraints (zeta, theta, phi, etc.) remain the same as in your original code, replacing solver.add_clause with cnf_clauses.append
        ...
        for i in range(1, self.max_adder + 1):
            ...
            for k in range(self.adder_wordlength - 1):
                for j in range(self.adder_wordlength - 1 - k):
                    cnf_clauses.append([-v2i(('zeta', i, k)), -v2i(('z', i, j + k)), v2i(('c', i, j))])
                    cnf_clauses.append([-v2i(('zeta', i, k)), v2i(('z', i, j + k)), -v2i(('c', i, j))])
            ...
            for kf in range(1, self.adder_wordlength - 1):
                for b in range(kf):
                    cnf_clauses.append([-v2i(('zeta', i, kf)), -v2i(('z', i, self.adder_wordlength - 1)), v2i(('c', i, self.adder_wordlength - 2 - b))])
                    cnf_clauses.append([-v2i(('zeta', i, kf)), v2i(('z', i, self.adder_wordlength - 1)), -v2i(('c', i, self.adder_wordlength - 2 - b))])
                    cnf_clauses.append([-v2i(('zeta', i, kf)), -v2i(('z', i, b))])

            cnf_clauses.append([-v2i(('z', i, self.adder_wordlength - 1)), v2i(('c', i, self.adder_wordlength - 1))])
            cnf_clauses.append([v2i(('z', i, self.adder_wordlength - 1)), -v2i(('c', i, self.adder_wordlength - 1))])
        ...
        for m in range(half_order + 1):
            ...
            for i in range(self.max_adder + 2):
                for word in range(self.adder_wordlength):
                    cnf_clauses.append([-v2i(('theta', i, m)), -v2i(('iota', m)), -v2i(('c', i, word)), v2i(('t', m, word))])
                    cnf_clauses.append([-v2i(('theta', i, m)), -v2i(('iota', m)), v2i(('c', i, word)), -v2i(('t', m, word))])

                theta_or.append(v2i(('theta', i, m)))
            
            # Ensure that at least one `theta[i][m]` is true per `i`
            cnf_clauses.append(theta_or)
        ...
        for m in range(half_order + 1):
            ...
            for k in range(self.adder_wordlength - 1):
                for j in range(self.adder_wordlength - 1 - k):
                    cnf_clauses.append([-v2i(('phi', m, k)), -v2i(('t', m, j)), v2i(('o', m, j + k))])
                    cnf_clauses.append([-v2i(('phi', m, k)), v2i(('t', m, j)), -v2i(('o', m, j + k))])
            ...
            for kf in range(1, self.adder_wordlength - 1):
                for b in range(kf):
                    cnf_clauses.append([-v2i(('phi', m, kf)), -v2i(('o', m, b))])
                    cnf_clauses.append([-v2i(('phi', m, kf)), -v2i(('t', m, self.adder_wordlength - 1)), v2i(('t', m, self.adder_wordlength - 2 - b))])
                    cnf_clauses.append([-v2i(('phi', m, kf)), v2i(('t', m, self.adder_wordlength - 1)), -v2i(('t', m, self.adder_wordlength - 2 - b))])

            cnf_clauses.append([-v2i(('t', m, self.adder_wordlength - 1)), v2i(('o', m, self.adder_wordlength - 1))])
            cnf_clauses.append([v2i(('t', m, self.adder_wordlength - 1)), -v2i(('o', m, self.adder_wordlength - 1))])
        
        #xor
        for m in range(half_order + 1):
            for w in range(self.adder_wordlength):
                cnf_clauses.append([v2i(('o', m, w)), v2i(('rho', m)), -v2i(('o_xor', m, w))])
                cnf_clauses.append([v2i(('o', m, w)), -v2i(('rho', m)), v2i(('o_xor', m, w))])
                cnf_clauses.append([-v2i(('o', m, w)), v2i(('rho', m)), v2i(('o_xor', m, w))])
                cnf_clauses.append([-v2i(('o', m, w)), -v2i(('rho', m)), -v2i(('o_xor', m, w))])
        ...
        for m in range(half_order + 1):
            ...
            cnf_clauses.append([v2i(('o_xor', m, 0)), v2i(('rho', m)), -v2i(('h_ext', m, 0))])
            cnf_clauses.append([v2i(('o_xor', m, 0)), -v2i(('rho', m)), v2i(('h_ext', m, 0))])
            cnf_clauses.append([-v2i(('o_xor', m, 0)), v2i(('rho', m)), v2i(('h_ext', m, 0))])
            cnf_clauses.append([-v2i(('o_xor', m, 0)), -v2i(('rho', m)), -v2i(('h_ext', m, 0))])

            # Carry out for w = 0
            cnf_clauses.append([v2i(('o_xor', m, 0)), -v2i(('cout_res', m, 0))])
            cnf_clauses.append([-v2i(('o_xor', m, 0)), -v2i(('rho', m)), v2i(('cout_res', m, 0))])
            cnf_clauses.append([v2i(('o_xor', m, 0)), v2i(('rho', m)), -v2i(('cout_res', m, 0))])
            cnf_clauses.append([v2i(('rho', m)), -v2i(('cout_res', m, 0))])

            # Ripple carry half adder for w > 0
            for w in range(1, self.adder_wordlength):
                cnf_clauses.append([v2i(('o_xor', m, w)), v2i(('cout_res', m, w - 1)), -v2i(('h_ext', m, w))])
                cnf_clauses.append([v2i(('o_xor', m, w)), -v2i(('cout_res', m, w - 1)), v2i(('h_ext', m, w))])
                cnf_clauses.append([-v2i(('o_xor', m, w)), v2i(('cout_res', m, w - 1)), v2i(('h_ext', m, w))])
                cnf_clauses.append([-v2i(('o_xor', m, w)), -v2i(('cout_res', m, w - 1)), -v2i(('h_ext', m, w))])

                # Carry out bits
                cnf_clauses.append([v2i(('o_xor', m, w)), -v2i(('cout_res', m, w))])
                cnf_clauses.append([-v2i(('o_xor', m, w)), -v2i(('cout_res', m, w - 1)), v2i(('cout_res', m, w))])
                cnf_clauses.append([v2i(('o_xor', m, w)), v2i(('cout_res', m, w - 1)), -v2i(('cout_res', m, w))])
                cnf_clauses.append([v2i(('cout_res', m, w - 1)), -v2i(('cout_res', m, w))])

        # Connect h_ext to h
        for m in range(half_order + 1):
            for word in range(self.adder_wordlength):
                if word <= self.wordlength - 1:
                    cnf_clauses.append([v2i(('h', m, word)), -v2i(('h_ext', m, word))])
                    cnf_clauses.append([-v2i(('h', m, word)), v2i(('h_ext', m, word))])
                else:
                    cnf_clauses.append([v2i(('h', m, self.wordlength - 1)), -v2i(('h_ext', m, word))])
                    cnf_clauses.append([-v2i(('h', m, self.wordlength - 1)), v2i(('h_ext', m, word))])
        ...
        if self.adder_depth > 0:
            for i in range(1, self.max_adder + 1):
                ...
                cnf_clauses.append([-v2i(('psi_alpha', i, 0)), v2i(('alpha', i, 0))])
                cnf_clauses.append([-v2i(('psi_beta', i, 0)), v2i(('beta', i, 0))])
                ...
                for d in range(1, self.adder_depth):
                    for a in range(1, i):
                        cnf_clauses.append([-v2i(('psi_alpha', i, d)), v2i(('alpha', i, a))])
                        cnf_clauses.append([-v2i(('psi_alpha', i, d)), v2i(('psi_alpha', a, d - 1))])

                        cnf_clauses.append([-v2i(('psi_beta', i, d)), v2i(('beta', i, a))])
                        cnf_clauses.append([-v2i(('psi_beta', i, d)), v2i(('psi_beta', a, d - 1))])
        ...
        # Now, write the CNF to a file
        num_vars = pb2cnf.var-1
