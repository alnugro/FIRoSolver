...
class FIRFilterZ3:
    def __init__(self, filter_type, order_upper, freqx_axis, freq_upper, freq_lower, ignore_lowerbound, adder_count, wordlength, app=None):
        ...
        self.gain_wordlength=6 #6 bits wordlength for gain
        self.gain_intW = 3
    ...
    def runsolver(self):
        ...
        r2b = Rat2bool()
        #round the bounds to closest fracW
        self.freq_upper_lin = [(r2b.frac2round([f],self.wordlength, self.coef_fracW)[0])*(2**self.coef_fracW) if not np.isnan(f) else np.nan for f in self.freq_upper_lin]
        self.freq_lower_lin = [(r2b.frac2round([f],self.wordlength, self.coef_fracW)[0])*(2**self.coef_fracW) if not np.isnan(f) else np.nan for f in self.freq_lower_lin]
        
        #multiply with the left hand fracW
        self.freq_upper_lin_prod = [f*2**(self.fracW-self.gain_fracW) if not np.isnan(f) else np.nan for f in self.freq_upper_lin]
        self.freq_lower_lin_prod = [f*2**(self.fracW-self.gain_fracW) if not np.isnan(f) else np.nan for f in self.freq_lower_lin]
        ...
        self.ignore_lowerbound_lin = int((r2b.frac2round([self.ignore_lowerbound_lin],self.wordlength, self.coef_fracW)[0])*(2**self.coef_fracW)*2**(self.fracW-self.gain_fracW))
        ...
        #bounds the gain
        self.gain_upperbound_int = int((r2b.frac2round([self.gain_upperbound],self.gain_wordlength, self.gain_fracW)[0])*(2**self.gain_fracW))
        self.gain_lowerbound_int = int((r2b.frac2round([self.gain_lowerbound],self.gain_wordlength, self.gain_fracW)[0])*(2**self.gain_fracW))
        ...
        for omega in range(len(self.freqx_axis)):
            ...
            for m in range(half_order+1):
                ...
                for w in range(self.wordlength):
                    if w==self.wordlength-1:
                        cm_word_prod = int((r2b.frac2round([cm],self.wordlength, self.coef_fracW)[0])*(2**self.coef_fracW)*(2**w)*-1)
                        # cm_word_prod= int(cm*(10** self.coef_accuracy)*(-1*(2**w))*(2**self.fg))
                    else: cm_word_prod= int((r2b.frac2round([cm],self.wordlength, self.coef_fracW)[0])*(2**self.coef_fracW)*(2**w))
            ...
            for g in range(self.gain_wordlength):
                gain_upper_prod = int(-1 * (2**g) * self.freq_upper_lin_prod[omega])
                ...
                if self.freq_lower_lin[omega] < self.ignore_lowerbound_lin:
                    gain_lower_prod=int((2**g) * self.freq_upper_lin_prod[omega])
    ...
    def plot_result(self, result_coef):
        ...
        self.freq_upper_lin = (freq_upper_lin_array * (2**-self.coef_fracW) * self.gain_res).tolist()
        self.freq_lower_lin = (freq_lower_lin_array * (2**-self.coef_fracW) * self.gain_res).tolist()
