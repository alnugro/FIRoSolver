class VariableMapper:
    def __init__(self, half_order, wordlength, gain_wordlength, adder_count):
        self.variables = self._initialize_variables(half_order, wordlength, gain_wordlength, adder_count)
    ...
    def _initialize_variables(self, half_order, wordlength, gain_wordlength, adder_count):
        variables = {
            ('h', a, w): f'h{a}_{w}' for a in range(half_order + 1) for w in range(wordlength)
        }
        variables.update({
            ('gain', g): f'gain{g}' for g in range(gain_wordlength)
        })
        variables.update({
...
gain_wordlength = 6
adder_count = 5
var_mapper = VariableMapper(half_order, wordlength, gain_wordlength, adder_count)
