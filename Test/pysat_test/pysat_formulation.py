...
class FIRFilter:
    ...
    def runsolver(self):
        ...
        solver.add_atmost(gain_literals, self.gain_upperbound_int, gain_coeffs)
        solver.add_atmost(gain_literals, -1 * self.gain_lowerbound_int, inverted_gain_coeffs)
        ...
        for omega in range(len(self.freqx_axis)):
            ...
            solver.add_atmost(filter_upper_pb_literals, 0, filter_upper_pb_coeffs)
            ...
            solver.add_atmost(filter_lower_pb_literals, 0, [-coeff for coeff in filter_lower_pb_coeffs])
        ...
        for i in range(1, self.N + 1):
            ...
            for a in range(i):
                ...
                alpha_sum.append((v2i(('alpha', i, a)), 1))
                beta_sum.append((v2i(('Beta', i, a)), 1))

            solver.add_atmost([x[0] for x in alpha_sum], 1, [x[1] for x in alpha_sum])
            solver.add_atmost([x[0] for x in beta_sum], 1, [x[1] for x in beta_sum])
        ...
        for i in range(1, self.N + 1):
            ...
            for k in range(self.wordlength - 1):
                ...
                gamma_sum.append((v2i(('gamma', i, k)), 1))
            
            solver.add_atmost([x[0] for x in gamma_sum], 1, [x[1] for x in gamma_sum])
        ...
        for i in range(1, self.N + 1):
            ...
            for k in range(self.wordlength - 1):
                ...
                zeta_sum.append((v2i(('zeta', i, k)), 1))
            
            solver.add_atmost([x[0] for x in zeta_sum], 1, [x[1] for x in zeta_sum])
        ...
        for m in range(half_order + 1):
            ...
            e_sum.append((v2i(('e', m)), 1))
        
        solver.add_atmost([x[0] for x in e_sum], connected_coefficient, [x[1] for x in e_sum])
