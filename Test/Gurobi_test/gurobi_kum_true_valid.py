...
class FIRFilter:
    ...
    def runsolver(self):
        ...
        if m.status == GRB.OPTIMAL:
            ...
            self.validate(c_a, c_a_i, c_a_i_k, c_sh_a_i, sg_a_i, c_sh_sg_a_i, o_a_m_s_sg, sh_a_i_s, h_int)
        else:
    ...
    def validate(self, c_a, c_a_i, c_a_i_k, c_sh_a_i, sg_a_i, c_sh_sg_a_i, o_a_m_s_sg, sh_a_i_s, h_int):
        for a in range(0, self.A_M):
            ...
            for i in range(2):
                ...
                input_value = 0
                for k in range(a + 1):
                    if c_a_i_k[a, i, k].X > 0.5:
                        input_value = c_a[k].X if k > 0 else 1  # c_a[0] is constant 1
                        if i == 0:
            ...
            # Validate adder sums
            c_a_val_calc = sum([c_sh_sg_a_i[a, i].X for i in range(2)])
        ...
        for i in range(len(h_int)):
            for a in range(self.A_M + 1):
                for s in range(2 * self.wordlength + 1):
                    shift = s - self.wordlength
                    for sg in [0, 1]:
                        if o_a_m_s_sg[a, i, s, sg].X > 0.5:
                            c_a_val = c_a[a].X if a > 0 else 1  # c_a[0] is constant 1
                            calculated_value = (-1) ** sg * (2 ** shift) * c_a_val
                            sign = 1 if sg == 0 else -1
                            self._print(f'Output[{i}] is connected to adder {a + 1} with an adder value of {c_a_val}, end result is shifted by {shift}, thus it\'s multiplied by {2**shift} with sign of {sign}')
                            h_int_val = h_int[i].X
