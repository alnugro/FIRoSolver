...
class MainWindow(QMainWindow):
    ...
    def on_run_solver_but_click(self):
        ...
        # Set data
        self.magnitude_plotter.set_data(self.order_upper_box.value())
        self.solver.set_input_arg(self.input_parser_tuple())
        ...
        future = self.executor.submit(self.run_solver_in_thread, self.solver)
        future.add_done_callback(self.solver_done)

    def run_solver_in_thread(self, solver):
        self.solver.runsolver()
    ...
    def solver_done(self, future):
        ...
        try:
            ...
            self.live_logger(f"Error during result processing or plotting: {e}")
            print(f"Error during result processing or plotting: {e}")
            traceback.print_exc()
